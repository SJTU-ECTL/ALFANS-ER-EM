...
extern double real_er_min, ave_error_min;
extern int lit_save_pre;
...
#define flag_threshold_em_tmp 1 
#define flag_round 0 
...
void exdc_factor_new_v3_ave(BnetNetwork *net, DdManager **dd, char *cnode, double score_factor, vector<char*> &unsort_cutnodes, multimap<double, struct score_pla> &score_record, vector<string> &org_pla, vector<string> &dont_care, set<char*> &po_set, map<string, int> &internal_index, vector<struct index_flag> &input_index, map<string, struct wi_pair> &sim_output_wi, struct po_index_set &pis, vector<string> &rand, vector<string> &simu_res, int num_output, map<string, map<string, double> > &node_pattern_rate, struct score_pla &max_sp, double max_score,  double threshold_er, double threshold_em,  double real_em, int &min_modified_po, int &max_modified_po, int &flag_add_const, int iIndex)
{
	...
	int lit_save, lit_save_total, status = 0;
  	double score, real_er_whole, this_ave_error_mag = 0;
	double est_real_er_whole;
	...
	int real_aff_po_min, real_aff_po_max;
	...
	for(int i = 1; i <= num_lit; i++)
	{
		...
		//		cout << "real_er is beyond!" << endl;
			else
			{
				...
				flag_continue = 0;
				lit_save_total = lit_save;
				if (score_mode == 3)
				{
			      		if (this_real_er_case1 == 0 && status == 0)
						score = lit_save_total * (1e+5);
					else if (this_real_er_case1 == 0 && status == 1)
						score = lit_save_total / (cur_real_er_whole+exp_ratio-cur_real_er_whole*exp_ratio);
					else 
						score = lit_save_total / (cur_real_er_whole+this_real_er_case1-cur_real_er_whole*this_real_er_case1);
				...
				else if (score_mode == 4)
				{
					if (this_real_er_case1 == 0 && status == 0)
						...
						er_part = er_weight * (cur_real_er_whole+exp_ratio-cur_real_er_whole*exp_ratio)/ini_threshold_er;
					else
						er_part = er_weight * (cur_real_er_whole+this_real_er_case1-cur_real_er_whole*this_real_er_case1)/ini_threshold_er;
					score = lit_save_total/er_part;
                               		if (score > max_score) flag_continue = 1;
				}
				else if (score_mode == 6)
				{
					if (this_real_er_case1 == 0 && status == 0)
						er_part = 1e+8;
					else if (this_real_er_case1 == 0 && status == 1)
					{
						est_real_er_whole = cur_real_er_whole + exp_ratio - cur_real_er_whole * exp_ratio;
						er_part = est_real_er_whole / (ini_threshold_er - est_real_er_whole);
					}
					else
					{
						est_real_er_whole = cur_real_er_whole + this_real_er_case1 - cur_real_er_whole * this_real_er_case1;
						er_part = est_real_er_whole / (ini_threshold_er - est_real_er_whole);
					}
					score = lit_save_total / er_part;
					if (real_em > 2) score = score/(log(real_em)/log(2));
                               		if (score > max_score) flag_continue = 1;
				}
                                ...
                                if (flag_continue)	
				{
					...
					{
						if (flag_threshold_em_tmp && max_score > 0 && score_mode == 6)
						{
							cout << "max_score = " << max_score << endl;
							double em_part_threshold = lit_save / (er_part * max_score);
						//	threshold_em_tmp = est_real_er_whole * pow(2, em_part_threshold);
							threshold_em_tmp = pow(2, em_part_threshold);
							if (flag_add_const) threshold_em_tmp /= mul_const;
							cout << "threshold_em_tmp = " << threshold_em_tmp << endl;
							if (threshold_em_tmp > threshold_em) threshold_em_tmp = threshold_em;
						}
						real_aff_po_min = 1000;
						real_aff_po_max = -1;
						this_ave_error_mag = estimate_ave_error_mag_bdd(net, cnode, sub_abs_ckt, sub_abs_pi, sub_abs_po, this_min_modified_po, this_max_modified_po, final_org_pla, threshold_em_tmp, real_aff_po_min, real_aff_po_max, real_er_whole);
					}
					...
					{		
						...
						if (score_mode == 3)
						{
							if (real_er_whole == 0) score = 1e+8;
							else	score = lit_save_total / real_er_whole;
						}
						else if (score_mode == 4)
						{
							if (real_er_whole == 0) score = 1e+8;
							...
							{
								er_part = er_weight * real_er_whole / ini_threshold_er;
								em_part = em_weight * (this_ave_error_mag/real_er_whole) / ini_threshold_em;
								if (em_part < 0.05 * er_part)
								{
									cout << "unbalancing!" << endl;
									score = lit_save_total/er_part;
								}
								else score = lit_save_total/(er_part + em_part);	
								cout << "er_part = " << er_part << ", em_part = " << em_part << endl;
							}
						}
						else if (score_mode == 6)
						{
							if (real_er_whole == 0) score = lit_save * (1e+8);
							else
							{
								er_part = real_er_whole / (ini_threshold_er - real_er_whole);
							//	em_part = log(this_ave_error_mag/real_er_whole)/log(2);
							//	if (em_part == 0) em_part = 0.5;
								em_part = log(this_ave_error_mag)/log(2);	
								cout << "this_ave_error_mag = " << this_ave_error_mag << ", em_part = " << em_part << endl;
								if (this_ave_error_mag <= 1) 
								{
									em_part += log(10)/log(2);	
									if (!flag_add_const) flag_add_const = 1;
									cout << "em_part = " << em_part << endl;
								}
								score = lit_save_total /(er_part * em_part);
							}
						}
						else if (score_mode == 5)
							score = lit_save_total / (real_er_whole * this_ave_error_mag);
						cout << "this_score = " << score << ", max_score = " << max_score << endl; 
						struct score_pla can_sp;
						can_sp.node = cnode;
						can_sp.score = score;
						can_sp.lit_save = lit_save_total;
						can_sp.real_er = real_er_whole;
						can_sp.max_weight = max_weight;
						can_sp.ave_em = this_ave_error_mag;	
						can_sp.pla = final_org_pla;
						can_sp.status = status;
						can_sp.aff_po_min = min_modified_po; 
						can_sp.aff_po_max = max_modified_po;
						score_record.insert(make_pair(score, can_sp));
						if (score > max_score) 
						{
							cout << "find new max_score, cnode = " << cnode << endl;
						//	cout << "er_part = " << er_part << ", em_part = " << em_part << endl;
							cout << "lit_save_total = " << lit_save_total << ", real_er_whole = " << real_er_whole << ", ave_error = " << this_ave_error_mag << endl;
							cout << "er_part = " << er_part << ", em_part = " << em_part << endl;
							max_score = score;
							max_sp = can_sp;
			...
			if(this_real_er_case2 > er_margin)
			...
			else
			{
				...
				if (score_mode == 3)
				{
					if (this_real_er_case2 == 0 && status == 0)
						...
						score = lit_save_total / (cur_real_er_whole+this_real_er_case2-cur_real_er_whole*this_real_er_case2);
                                	if (score > max_score) flag_continue = 1;
				}
				else if (score_mode == 4)
				{
					if (this_real_er_case2 == 0 && status == 0)
						...
						er_part = er_weight * (cur_real_er_whole+this_real_er_case2-cur_real_er_whole*this_real_er_case2)/ini_threshold_er;
					score = lit_save_total/er_part;
                               		if (score > max_score) flag_continue = 1;
				}
				else if (score_mode == 6)
				{
					...
					{
						est_real_er_whole = cur_real_er_whole + this_real_er_case2 - cur_real_er_whole * this_real_er_case2;
						er_part = est_real_er_whole / (ini_threshold_er - est_real_er_whole);
					}
					score = lit_save_total / er_part;
					if (real_em > 2) score = score/(log(real_em)/log(2));
				...
				if (flag_continue)
				{
					...
					else 
					{
						if (flag_threshold_em_tmp && max_score > 0 && score_mode == 6)
						{
							double em_part_threshold = lit_save_total / (er_part * max_score);
						//	threshold_em_tmp = est_real_er_whole * pow(2, em_part_threshold);
							threshold_em_tmp = pow(2, em_part_threshold);
							if (flag_add_const) threshold_em_tmp /= mul_const;
							cout << "threshold_em_tmp = " << threshold_em_tmp << endl;
					...
					{
						cout << "within threshold_em_tmp!" << endl;
					/*	if (this_ave_error_mag > ave_error_max) ave_error_max = this_ave_error_mag;
						if (this_ave_error_mag < ave_error_min) ave_error_min = this_ave_error_mag;
						if (real_er_whole > real_er_max) real_er_max = real_er_whole;
						if (this_real_er_case2 < real_er_min) real_er_min = this_real_er_case2;
					*/
						if (score_mode == 3)
						{
							if (real_er_whole == 0) score = 1e+8;
				...
				else
				{
					...
					if (score_mode == 3)
					{
						if (this_real_er_case3 == 0 && status == 0)
							...
							score = lit_save_total / (cur_real_er_whole+this_real_er_case3-cur_real_er_whole*this_real_er_case3);
                                		if (score >  max_score) flag_continue = 1;
					}
					else if (score_mode == 4)
					{
						if (this_real_er_case3 == 0 && status == 0)
							...
							er_part = er_weight * (cur_real_er_whole+this_real_er_case3-cur_real_er_whole*this_real_er_case3)/ini_threshold_er;
						score = lit_save_total/er_part;
                                		if (score >  max_score) flag_continue = 1;
					}
					else if (score_mode == 6)
					{
						...
						{
							est_real_er_whole = cur_real_er_whole + this_real_er_case3 - cur_real_er_whole * this_real_er_case3;
							er_part = est_real_er_whole / (ini_threshold_er - est_real_er_whole) ;
						}
						score = lit_save_total / er_part;
						if (real_em > 2) score = score/(log(real_em)/log(2));
