...
int sample_num = 15000;
extern int sample_size;
extern int conf_prob;
...
//			diff_times_sampling_std = 2.58;		
		...
		{
			if (sample_times == 10)
				diff_times_sampling_std = 1.833;
			else if (sample_times == 20)
				diff_times_sampling_std = 1.729;
			else if (sample_times == 5)
				diff_times_sampling_std = 2.132;
			else
		...
		double lb_ave_error_mag = sampling_mean - diff_times_sampling_std * S_std;
		double ub_ave_error_mag = sampling_mean + diff_times_sampling_std * S_std;
	//	ave_error_mag = ub_ave_error_mag;
		ave_error_mag = sampling_mean;
...
void simu_ckt_both_after_modify(BnetNetwork *net, int &num_output, string cnode, struct score_pla &max_sp, int num_lines_org_pla, vector<string> &simu_res_after_modify, int iIndex)
{
	...
	*/
		...
		int num_input = net->npis;
		gen_testbench_org_after_modify(num_input, num_output, sample_num, max_sp.status);
	...
	else
	{
		...
		*/
		...
		gen_testbench_sim_after_modify(num_input, num_output, sample_num, max_sp.status, dut);
