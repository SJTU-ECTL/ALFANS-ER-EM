...
#define exp_ratio 0.002
#define em_ratio 1000
...
void exdc_factor_new_v2(BnetNetwork *net, DdManager **dd, char *cnode, vector<char*> &unsort_cutnodes, map<string, struct score_pla> &sim_record, vector<string> &org_pla, vector<string> &dont_care, vector<string> &local_dc_set, set<char*> &po_set, map<string, vector<string> > &po_dont_care_map, map<string, int> &internal_index, vector<struct index_flag> &input_index, map<string, struct wi_pair> &sim_output_wi, struct po_index_set &pis, vector<string> &rand, vector<string> &simu_res, int num_output, map<string, map<string, double> > &node_pattern_rate, struct score_pla &max_sp, double threshold_er, double threshold_em, int iIndex)
{
    ...
    int lit_save;
  	...
  	double max_error_mag = 1; 
  	int num_lines_org_pla = 0;
  	int flag_within = 0;
	...
	if (po_set.size() == 1) 
	{
		cout << "Current node has only one affected po!" << endl;
...
/*	if (max_error_mag/sample_num > em_ratio * threshold_em)
	{
		cout << "estimated error magnitude is not within threshold_em!" << endl;
		return; 
	}
*/
...
//	cout << "patterns with error rate <= threshold_er: " << endl;
	...
	for(itrm_sd = pattern_rate.begin(); itrm_sd != pattern_rate.end(); itrm_sd++)
	{
		...
		{
		//	cout << itrm_sd->first << ": " << er << endl;
			num_useful++;
		}
	}
	if(!num_useful && dont_care.empty())
	{
		max_sp.score = -1;
		max_sp.real_er = 1;
	...
	double er_devi;
	if(num_lit > 5)
		num_lit = 5;           //set the limit for literal save
	...
	//			cout << *itrs << endl;
...
#ifdef use_bdd
			int res = exdc_cubes_real_er(net, dd, dont_care, local_dc_set, unsort_cutnodes, all_exdc_minterms, pattern_er, this_real_er, threshold_er);
			...
			/*	if (this_real_er * max_error_mag > em_ratio * threshold_em)
					cout << "out of threshold_em!" << endl;	
			    else
			*/
			    {
					double cur_ave_error_mag;
					if(this_simu_er == 0) 
					{
						cur_ave_error_mag = 0;
						cout << "po_set: " << endl;
						for(itrs_char = po_set.begin(); itrs_char != po_set.end(); itrs_char++)
						{
							...
							if(status)
							{
								...
								itrm_sw = sim_output_wi.find(po);
								int weight = itrm_sw->second.weight;
								cur_ave_error_mag += pow(2.0, weight) * exp_ratio;
							}
						}
						cout << endl;
					}
					else
						cur_ave_error_mag = estimate_ave_error_mag(po_set, po_dont_care_map, sim_output_wi, input_index, rand, simu_res, dont_care, local_dc_set, all_exdc_minterms, iIndex);
					cout << "cur_ave_error_mag = " << cur_ave_error_mag << endl;
					cout << "threshold_em = " << threshold_em << endl;
	#ifdef not_care_em
					cur_ave_error_mag = -1;
	#endif
					if(cur_ave_error_mag <= em_ratio * threshold_em)
					{
						...
						if(this_real_er == 0 && cur_ave_error_mag == 0)
							score = lit_save * 100;
						else					
						//	score = lit_save * (er_weight*(threshold_er - this_real_er)/threshold_er + em_weight*(threshold_em - cur_ave_error_mag)/threshold_em);
							score = lit_weight * lit_save/num_lit + er_weight*(threshold_er - this_real_er)/threshold_er + em_weight*(threshold_em - cur_ave_error_mag)/threshold_em;
						...
						if(score > max_score)
						{
							...
							max_sp.ave_em = cur_ave_error_mag;
...
#ifdef use_bdd			
			res = exdc_cubes_real_er(net, dd, dont_care, local_dc_set, unsort_cutnodes, all_exdc_minterms, pattern_er, this_real_er, threshold_er);
			cout << "this_real_er = " << this_real_er << endl; 
#endif
#ifdef use_simu
			this_simu_er = 0;	
			...
			*/
			    {
					...
					if(cur_ave_error_mag <= em_ratio * threshold_em)
					{
						...
						//	score = lit_save / (this_real_er * cur_ave_error_mag);
						//	score = lit_save * (er_weight*(threshold_er - this_real_er)/threshold_er + em_weight*(threshold_em - cur_ave_error_mag)/threshold_em);
							score = lit_weight * lit_save/num_lit + er_weight*(threshold_er - this_real_er)/threshold_er + em_weight*(threshold_em - cur_ave_error_mag)/threshold_em;
		...
		int lit_save = i;
		cout << endl << "##lit_save = " << i << endl;
		double num_choice = (factorial(num_lit) / factorial(i) ) / factorial(num_lit - i);
	...
	cout << "max_sp: " << endl;
	cout << "score : " << max_sp.score << ", real_er: " << max_sp.real_er << ", ave_em = " << max_sp.ave_em << endl;
	
	if (flag_within && max_sp.ave_em != 0 && po_set.size() > 1)
	{
		vector<string> simu_res_after_modify;
		cout << endl << "1. enter into simu_ckt_both_after_modify!" << endl;
		string cur_node(cnode);
		simu_ckt_both_after_modify(net, num_output, cur_node, max_sp, num_lines_org_pla, simu_res_after_modify, iIndex);
		cout << "2. enter into get_ave_error_mag_after_modify!" << endl;
		double real_ave_error_mag = get_ave_error_mag_after_modify(simu_res, simu_res_after_modify, pis, iIndex);
		cout << endl << "real_ave_error_mag = " << real_ave_error_mag << endl;
		double est_ave_error_mag = max_sp.ave_em + ini_threshold_em - threshold_em;
		cout << "est_ave_error_mag = " << max_sp.ave_em << " + " << ini_threshold_em - threshold_em << " = " << max_sp.ave_em + ini_threshold_em - threshold_em << endl;
		if (real_ave_error_mag > est_ave_error_mag)
		{
			cout << "error!" << endl;
		//	exit(2);
		} 
		if (real_ave_error_mag == est_ave_error_mag)
		{
			cout << "real = est!" << endl;
			if (po_set.size() == 1)
				cout << "only one affected po!" << endl;
			else
				cout << "more than one affected po!" << endl; 
		}
		if (po_set.size() == 1)
		{
			if (real_ave_error_mag == est_ave_error_mag)
			{
				cout << "when po_set.size() = 1, real = est!" << endl;
				num_one_po_equal++;
			}
			else
			{
		...
		if (real_ave_error_mag > ini_threshold_em)
		{
			max_sp.score = -1;
			return;
		}
		
		if (real_ave_error_mag != max_sp.ave_em)
		{
			cout << "old max_sp.score = " << max_sp.score << endl;
			max_sp.ave_em = real_ave_error_mag;
			max_sp.score = lit_weight*max_sp.lit_save/num_lit + er_weight*(threshold_er - max_sp.real_er)/threshold_er + em_weight*(threshold_em - max_sp.ave_em)/threshold_em;
			cout << "new max_sp.score = " << max_sp.score << endl;
		}
	}
