...
#define flag_em_p 1 
#define flag_er_part 2 
#define flag_decoup 0 
...
void exdc_factor_new_v3_ave(BnetNetwork *net, DdManager **dd, char *cnode, double score_factor, vector<char*> &unsort_cutnodes, multimap<double, struct score_pla> &score_record, vector<string> &org_pla, vector<string> &dont_care, set<char*> &po_set, map<string, int> &internal_index, vector<struct index_flag> &input_index, map<string, struct wi_pair> &sim_output_wi, struct po_index_set &pis, vector<string> &rand, vector<string> &simu_res, int num_output, map<string, map<string, double> > &node_pattern_rate, struct score_pla &max_sp, double max_score,  double threshold_er, double threshold_em,  double real_em, int &min_modified_po, int &max_modified_po, double &T_em, double &T_er, int &flag_add_const, int iIndex)
{
	...
	double est_real_er_whole, est_er_margin, acc_er_margin;
	...
	int real_aff_po_max, real_aff_po_min;
			...
			//	if (this_real_er_case1/er_margin > 0.9) this_real_er_case1 *= pow(ratio_pen, ini_threshold_er/threshold_er);
				...
				if (score_mode == 6)
				{
					...
					else
					{
						...
						if (flag_er_part == 1) er_part = this_real_er_case1;		
						else if (flag_er_part == 2)
							er_part = est_real_er_whole / (ini_threshold_er - est_real_er_whole);
					}
					est_er_margin = (ini_threshold_er - est_real_er_whole) / (1 - est_real_er_whole);
					score = lit_save_total / er_part;
					if (real_em > 2)
					{
						double tmp;
						if (flag_decoup) 		
							tmp = log(0.5 * real_em / est_real_er_whole)/log(2);
						else
							tmp = log(0.5 * real_em)/log(2);
						em_part = pow(e, tmp/T_em);
						if (flag_em_p)
							score = score/em_part;
					}
                                ...
                                if (flag_continue)	
				{
					...
					{
						if (flag_threshold_em_tmp && max_score > 0)
						{
						...
						//	threshold_em_tmp = est_real_er_whole * pow(2, em_part_threshold);
							threshold_em_tmp = pow(2, T_em * log(em_part_threshold));
					...
					{		
						...
						if (score_mode == 6)
						{
							...
							//	if (em_part == 0) em_part = 0.5;
								double tmp;
								if (flag_decoup)
									tmp = log(this_ave_error_mag/real_er_whole)/log(2);
								else
								 	tmp = log(this_ave_error_mag)/log(2);
								em_part = pow(e, tmp/T_em);	
								score = lit_save_total /er_part;
								if (flag_em_p)
									score = score / em_part;
						...
						if (score > max_score) 
						{
							cout << endl << "#find new max_score, cnode = " << cnode << endl;
							cout << "lit_save = " << lit_save_total << ", real_er_whole = " << real_er_whole << ", ave_error = " << this_ave_error_mag << endl;
						...
						if (score_mode == 6)
						{
							...
							else
							{
								if (flag_er_part == 1) er_part = this_real_er_case2;		
									...
									er_part = real_er_whole / (ini_threshold_er - real_er_whole);
							//	er_part = real_er_whole - cur_real_er_whole;
							//	em_part = log(this_ave_error_mag/real_er_whole)/log(2);
							//	if (em_part == 0) em_part = 0.5;
								double tmp;
								if (flag_decoup)
				...
				else
				{
					...
					if (score_mode == 6)
					{
						...
						else
						{
							...
							if (flag_er_part == 1) er_part = this_real_er_case3;		
							else if (flag_er_part == 2)
							er_part = est_real_er_whole / (ini_threshold_er - est_real_er_whole) ;
						//	er_part = est_real_er_whole - cur_real_er_whole;
						}
...
//	max_modified_po = log(max_sp.max_em)/log(2);    
	...
	cout << "score : " << max_sp.score << ", real_er: " << max_sp.real_er << endl;
