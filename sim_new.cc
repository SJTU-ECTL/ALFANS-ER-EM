...
char *find_exdc_sim(BnetNetwork *net, DdManager **dd, BnetNetwork *net_comb, DdManager **dd_comb,  vector<string> &final_pla, map<string, struct score_pla> &sim_record, map<string, int> &internal_index, map<string, struct wi_pair> &sim_output_wi, struct po_index_set &pis, vector<string> &rand, vector<string> &simu_res, int &num_output, map<string, map<string, double> > &node_pattern_rate, double &max_score, int &ave_em, double threshold_er, double threshold_em, double real_em, int &min_modified_po, int &max_modified_po, int iIndex)
{
    ...
    for(itrm_dc = ave_sp.begin(); itrm_dc != ave_sp.end(); itrm_dc++, index++)
    {
        ...
        cout << endl <<"--------------------------------------------" << endl;  
        cout << "%%exdc for " << cnode << ", index = " << index << ", rp = " << nd->rp << endl; 
...
/*    	cout << endl << "a0. Run mvsis!" << endl;
    	...
    	vector<string> dont_care, org_pla, insig_string;
    	BnetTabline *t = nd->f;
		while(t != NULL) 
		{
			string str(t->values);
    	...
    	read_mvsis_result(dont_care, insig_string, filename); 
...
*/
		...
		vector<string> dont_care, org_pla;
		BnetTabline *t = nd->f;
		while(t != NULL) 
		{
			string str(t->values);
...
/*    	for(itrs = po_set.begin(); itrs != po_set.end(); itrs++)
    	{
    		char *po = *itrs;
    	//	cout << "po = " << po << endl;
			write_mvsis_rug_po(po, cnode);
			char com[100];
			sprintf(com, "echo /dev/null > mvsis_po.txt; mvsis -t none -f ./script/mvsis_dc_po.rug > mvsis_po.txt");
    		system(com);  
    		vector<string> dont_care_po, insig_string_po;
    		string filename = "mvsis_po.txt";
    		read_mvsis_result(dont_care_po, insig_string_po, filename); 
    		if(!dont_care_po.empty()) 			
		    {	    	
		    //	cout << "dont_care_po is not empty!" << endl;
		    	int res = permute(dont_care_po, insig_string_po, unsort_cutnodes);
		    	if(res)
		    	{
		    		cout << "mvsis error!" << endl;
		    		dont_care_po.clear();
